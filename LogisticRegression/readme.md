...
In this instance, we want to check the type of apple based on weight. I have included 4 apples varying from 30 grams to 300 grams (crab between 30 and 65, fuji & gala between 65 and 180, and granny smith between 180 and 300). To denote type in our dataset, any crab apple is assigned a value of 1, and all others 0, in a 3rd column. This is because the method only works on a binary. We cannot feed it text, only numbers, and our target must be 1 or 0. We also need to drop any values we are not measuring, so the id and apple columns need to go. You can use more values for training, which will improve the quality of your model, but here we just need 2.
...
Accuracy is the measure of how well our model compares to the actual test data, up to 100%, or 1. In this instance, with this dataset, our accuracy is 1, meaning our model is able to perfectly predict which apple is a crab apple. Keep in mind, however, that crab apples in this dataset do not share weight with any other apple, making the process rather easy. We would see the same result with granny smith apples, just in the upper range of weight, for the same reason.

By contrast, if we do the same for gala apples, our accuracy drops to 76%. This is because it shares the weight range with fuji apples, and with only weight to use in it's predictions, the model is less certain. It is still somewhat capable, as there are only two options to choose from in this range, but it is a noteworthy drop in performance.
